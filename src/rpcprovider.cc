 ...
 * service描述    对应   service* 对象
 * service* 对象  对应   很多method方法对象（method_name）
*/
...
void RpcProvider::NotifyService(google::protobuf::Service* service)
{
    ...
    {
        // 获取服务对象指定下标的服务方法描述（抽象描述 Service描述对象 MethodDescriptor描述方法）
...
void RpcProvider::Run()
{
    ...
    // session timeout 30s   zkclient的网络IO线程会以1/3的timeout时间发送ping消息
    ZkClient zkCli;
    zkCli.Start();
 ...
 * service_name method_name args_size  定义proto的message类型，进行数据的序列化和反序列化
 * 
 * header_size(4字节) + header_str(service_name method_name args_size) + args_str
*/
// 已建立连接用户的读写事件回调 如果远端有一个rpc服务调用请求，该方法就会响应
                            ...
                            Timestamp timestamp)
{
    ...
    // 给下面method方法的调用，绑定一个Closure的回调函数 done->run()
